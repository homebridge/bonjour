...
Browser.prototype.start = function () {
  ...
  this._onresponse = function (packet, rinfo) {
    ...
    // Update the hostname-to-addresses cache with any A/AAAA records in this
    // packet. This handles the case where a DNS response spans multiple packets
    // and address records only appear in some of them.
    packet.answers.concat(packet.additionals).forEach(function (rr) {
      if ((rr.type === 'A' || rr.type === 'AAAA') && rr.ttl > 0) {
        const host = rr.name.toLowerCase()
        if (!self._hostnameToAddresses[host]) {
          self._hostnameToAddresses[host] = []
        }
        if (!self._hostnameToAddresses[host].includes(rr.data)) {
          self._hostnameToAddresses[host].push(rr.data)
        }
      }
    })
    ...
    Object.keys(nameMap).forEach(function (name) {
      ...
      const matches = buildServicesFor(name, packet, self._txt, rinfo, self._hostnameToAddresses)
...
function buildServicesFor (name, packet, txt, referer, hostnameToAddresses) {
  ...
  return records
    ...
    .map(function (ptr) {
      ...
      // If no addresses were found in this packet, fall back to the cache.
      // This handles multi-packet responses where address records may only
      // appear in earlier packets.
      if (service.addresses.length === 0 && hostnameToAddresses && service.host) {
        const cached = hostnameToAddresses[service.host.toLowerCase()]
        if (cached) service.addresses = cached.slice()
      }
